...
- **You dispatch; the `orchestrator` subagent works.** Each time the loop re-feeds you, spawn
  `orchestrator` for one iteration and relay its ≤10-line summary. Do not implement items, read
  review files, or run Gradle in this session — your context is the expensive, long-lived one.
...
- The orchestrator spawns the relevant specialists (cap 3–4) to review; they recommend, only the
  orchestrator edits.
