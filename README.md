...
- **Bot goes silent when it needs a decision (clarifying questions / option pickers never arrive)** — Claude used an interactive terminal prompt (e.g. the `AskUserQuestion` tool or a plan-approval dialog). Those render only in the terminal UI and are never delivered to Telegram, so the session blocks waiting for input that can't arrive. The workspace `CLAUDE.md` template now forbids this and tells Claude to ask via a plain `reply` with numbered options instead. To update an existing install, re-run `setup-vps.sh` — it detects that the template changed and asks whether to overwrite (your current file is backed up to `CLAUDE.md.bak`; answering no saves the new template as `CLAUDE.md.new` for manual merge). If it happens live, attach to the session (`tmux attach -t claude`) and answer the prompt there once.
